...
const SKIP_CRATES = new Set(['std', 'core', 'alloc', 'self', 'super', 'crate']);
 ...
 */
async function getCrateSourcePaths(workspacePath: string): Promise<Map<string, string>> {
...
const SERDE_ATTR_REGEX = /#\[serde\(([^)]*)\)\]/g;
...
function computeSerdeVariants(
  ...
  const variantRegex = /(?:#\[serde\(([^)]*)\)\]\s*)?(\w+)(?:\s*\{[^}]*\}|\s*\([^)]*\))?\s*,/g;
...
export async function resolveExternalTypes(
  ...
  for (const usage of externalUses) {
    ...
    const serdeVariants = computeSerdeVariants(found.definition, found.serdeAttributes);
 ...
 */
export function formatExternalTypesForPrompt(types: ResolvedExternalType[]): string {
